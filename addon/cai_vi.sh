...
echo "Hoàn tất. Bạn nên khởi động lại hệ thống để áp dụng tất cả thay đổi."

# Cài đặt tài liệu hỗ trợ cho các gói thông dụng
echo "Cài đặt các tài liệu hỗ trợ (help docs)..."
sudo apt install -y manpages-vi manpages-posix-vi
sudo apt install -y linux-doc-vi manpages-vi-extra debian-reference-vi apt-transport-https-vi tzdata-vi
